...
import com.epam.timetracking.services.database.util.*;
...
public class DBAdminService implements AdminService {
    ...
    public List<UserRequest> getUserRequests() throws ServiceException {
        try (Connection connection = dataSource.getConnection()) {
            ...
            List<UserRequest> userAppendTimeRequests =
                    jdbcTemplate.query(connection, SQLQueries.SELECT_USERS_APPEND_TIME_REQUESTS,
                            new ResultSetRowMapper<>(UserRequest.class));
            ...
            userRequests.addAll(userAppendTimeRequests);
    ...
    @Override
    public void processUserRequest(String requestType, int requestId, String choose, String comment) throws ServiceException {
        log.trace("requestType: " + requestType + " requestId: " + requestId + " choose: " + choose + " comment: " + comment);
        switch (requestType) {
            case "create NEW":
                addNewActivity(requestId, choose, comment);
                break;
            case "delete":
                deleteActivity(requestId, choose, comment);
                break;
            case "append time":
                appendTime(requestId, choose, comment);
                break;
            default:
    ...
    private void appendTime(int requestId, String choose, String comment) throws ServiceException {
        Integer[] reqIdParam = new Integer[]{requestId};
        Connection connection = null;
        try {
            connection = dataSource.getConnection();
            if ("confirm".equals(choose)) {
                log.debug("appendTime - start confirm transaction");
                Long resultTime = getResultTime(connection, reqIdParam);
                jdbcTemplate.update(connection, SQLQueries.APPEND_TIME_ACTIVITY, new Object[]{resultTime, requestId, requestId});
                jdbcTemplate.update(connection, SQLQueries.INSERT_CREATE_ACTIVITY_STORY_APPEND_TIME, reqIdParam);
                jdbcTemplate.update(connection, SQLQueries.DELETE_USER_REQUEST_APPEND_TIME_ACTIVITY_FROM_UPDATE_REQUESTS, reqIdParam);
                connection.commit();
            } else if ("deny".equals(choose)) {
                log.debug("appendTime - start deny transaction");
                jdbcTemplate.update(connection, SQLQueries.INSERT_MESSAGE_DENY_APPEND_TIME_ACTIVITY_REQUEST, new Object[]{3, comment, requestId});
                jdbcTemplate.update(connection, SQLQueries.SET_OLD_STATUS_ACTIVITY_FROM_APPEND_TIME_REQUESTS, new Integer[]{requestId, requestId});
                jdbcTemplate.update(connection, SQLQueries.DELETE_USER_REQUEST_APPEND_TIME_ACTIVITY_FROM_UPDATE_REQUESTS, reqIdParam);
                connection.commit();
            }
        } catch (SQLException exception) {
            rollbackTransaction(connection);
            log.error("Cannot append time activity: " + exception.getMessage(), exception);
            throw new ServiceException("Cannot append time activity: " + exception.getMessage(), exception);
        }
    }


    private Long getResultTime(Connection connection, Integer[] reqIdParam) throws SQLException {
        Long totalTime = jdbcTemplate.queryOne(connection,
                SQLQueries.SELECT_TOTAL_TIME_FROM_ACTIVITIES_BY_REQUEST,
                reqIdParam, Fields.getLongRowMapper(Fields.TOTAL_TIME));
        Long appendTime = jdbcTemplate.queryOne(connection, SQLQueries.SELECT_APPEND_TIME_FROM_REQUESTS_BY_ID,
                reqIdParam, Fields.getLongRowMapper(Fields.APPEND_TIME));
        return totalTime + appendTime;
    }
