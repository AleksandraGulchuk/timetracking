...
public class DBClientService implements ClientService {
    ...
    @Override
    public void sentRequestCreateActivity(User user, Activity activity) throws ServiceException {
    ...
    @Override
    public void sentRequestDeleteActivity(User user, Activity activity) throws ServiceException {
        Connection connection = null;
        try {
            connection = dataSource.getConnection();
    ...
    @Override
    public void sentRequestAppendActivityTime(User user, Activity activity) throws ServiceException {
        Connection connection = null;
        try {
            connection = dataSource.getConnection();
            log.debug("activity --> " + activity);
            int activityOldStatusId = getActivityStatusId(activity.getStatus(), connection);
            jdbcTemplate.update(connection, SQLQueries.INSERT_USER_APPEND_TIME_REQUEST,
                    new Object[]{
                            user.getId(),
                            activity.getId(),
                            activity.getAppendTime(),
                            activity.getComment(),
                            LocalDateTime.now(),
                            activityOldStatusId});
            jdbcTemplate.update(connection, SQLQueries.SET_STATUS_ON_UPDATE_ACTIVITY, new Integer[]{activity.getId()});
